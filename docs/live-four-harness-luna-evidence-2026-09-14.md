...
## Subsequent OMP run without Herdr

The separate OMP `process/v1` recipe passed its observed help/version contract and a Luna scratch activation after the JSONL parser was updated to require both an assistant `turn_end` and an `agent_end` when OMP omits `stopReason`. With `HERDR_ENV=0` and an empty `HERDR_PANE_ID`, brgr created a normal Git task worktree at `e98c6aefda15f1924b7661a255890bddea02d4d1`, not a Herdr worktree. Task `d09c900d-7aba-4419-96d0-a02f82e2bda3` requested `openai-codex/gpt-5.6-luna` at low effort and produced candidate result `47bf9169-fcf6-465f-9fd0-1a9b26951e13` containing exactly `BRGR_OMP_NO_HERDR_LUNA_20260914_62D1`.

Its artifact SHA-256 `ea4e546a71599e0928d9d6d7732e0c2a3e97b51a8c22656c76d7ad664c67dfd4` matched independent `shasum`; SQLite showed one unacknowledged owner inbox item before the Codex decision, then one decision and `acknowledged=1` after `brgr accept`. `brgr cleanup status` returned `no_owned_pane`. This proves the OMP process baseline can complete without Herdr on the locally modified brgr binary; it is not evidence that the older interactive `local.omp-herdr` adapter itself is Herdr-free. The new recipe and pinning changes require their own committed-SHA CI.
